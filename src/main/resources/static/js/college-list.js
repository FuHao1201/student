...
layui.use(['form' ,'table' ,'layer', 'laydate'], function() {
    ...
    function init() {
        ...
        // 单行监听
        _table.on('tool(college)', function(obj){
            var event = obj.event;
            var data = obj.data;
            if(event == 'reload') {
                obj = {page: 1}; //查询从首页开始
        ...
        //工具栏监听
        _table.on('toolbar(college)',function(obj){
            console.log(obj)
            var event = obj.event;
            var data = obj.data;
            if(event == 'add'){
    ...
    function add() { //新增
        _layer.open({
            ...
            content : '/collegeInfo/addOrUpdateCollegePage'
        })
    };
    function edit(id) { //编辑
        _layer.open({
            ...
            content : '/collegeInfo/addOrUpdateCollegePage?id='+ id
        });
    };
